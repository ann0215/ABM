"""
Explore the impact of the group-vision parameter (beta1) on the evacuation time of agent with different obstacles.
Run multiple iterations for each beta1 value ranging from 0.5 to 2, measuring the average evacuation times and CI.
...
beta_1_list = np.linspace(0.5,2,6)


for i in beta_1_list:
    
    result=intense_simulation(barrier_set, percent_threshold, 
                   r_1, r_2, r_3, A_p, A_o, B, i, beta2,
...
df=pd.DataFrame({'beta1':beta_1_list,'mean_time':mean_time_list,'ci_lower':ci_lower_list,'ci_upper':ci_upper_list,
...
plt.errorbar(df['beta1'], df['mean_time'], yerr=errors, fmt='o', capsize=5, capthick=2, color='blue', label='Mean Time with Barrier')
plt.errorbar(df['beta1'], df['nobarrier_time'], yerr=no_barrier_errors, fmt='o', capsize=5, capthick=2, color='red', label='Mean Time without Barrier')

plt.plot(df['beta1'], df['mean_time'], linestyle='-', marker='o', color='deepskyblue')
plt.plot(df['beta1'], df['nobarrier_time'], linestyle='-', marker='o', color='red')

plt.title('Average time via different group-vision')
plt.xlabel('group-vision beta1')
plt.ylabel('Average Time')
plt.grid(True, linestyle='--', alpha=0.6)
plt.legend()
plt.savefig('group-vision.png', dpi=150)
